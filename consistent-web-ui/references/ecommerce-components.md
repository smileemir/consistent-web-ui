...
| Dialog/drawer/dropdown | Title/purpose, content, actions, dismissal | Trigger/return focus, Escape where safe, outside click only when safe, scrolling, stacking, screen-reader role |
...
| Toast/status | One concise result and correction route if needed | Announce once; duplicate request; no fake success; avoid covering cart/CTA on phone |
